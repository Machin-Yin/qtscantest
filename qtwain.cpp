...
bool QTwain::my_sleSrc(TW_IDENTITY m_source)
{
    return (my_SleSrc(m_source));
}

QVector<TW_IDENTITY> QTwain::getScanSrc()
{
    QVector<TW_IDENTITY> srcArry;
    srcArry = GetScanSrc();
    return srcArry;
}

QStringList QTwain::getScanList()
{
    QStringList scanList = GetScanList();
    return scanList;
}

bool QTwain::initTwain(HWND hWnd)
{
    qDebug() << __FUNCTION__;
    bool isInit = InitTwain(hWnd);
    return isInit;
}
